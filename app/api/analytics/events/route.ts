...
import { trackAnalyticsEvent, resolvePublicContent } from "@/lib/analytics/service";
...
function limited(key: string) {
  const now = Date.now(); const current = rateLimit.get(key);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (eventName === "SEARCH_SUBMITTED") {
      ...
      if (queryLengthBucket !== "short" && queryLengthBucket !== "medium" && queryLengthBucket !== "long") return NextResponse.json({ accepted: false }, { status: 422 });
      await trackAnalyticsEvent({ eventName: AnalyticsEventName.SEARCH_SUBMITTED, eventCategory: AnalyticsEventCategory.PAGE, userId, anonymousId, sessionId, path, metadata: { queryLengthBucket } });
    ...
    if (content) await trackAnalyticsEvent({ eventName: content.eventName, eventCategory: AnalyticsEventCategory.CONTENT, userId, anonymousId, sessionId, path, contentType: content.contentType, contentId: content.contentId, metadata: { path } });
    else await trackAnalyticsEvent({ eventName: AnalyticsEventName.PAGE_VIEW, eventCategory: AnalyticsEventCategory.PAGE, userId, anonymousId, sessionId, path });
    return NextResponse.json({ accepted: true });
  } catch { return NextResponse.json({ accepted: true }); }
}
