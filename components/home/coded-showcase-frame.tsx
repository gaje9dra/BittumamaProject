...
export function SceneFrame({ children, active, index, previousIndex, reducedMotion, label }: SceneFrameProps) {
  ...
  return (
    <div
      ...
      aria-label={label}
      aria-hidden={!active}
      className={cn("absolute inset-0 overflow-hidden bg-[var(--dark-background)]", active ? "pointer-events-auto" : "pointer-events-none", reducedMotion ? (active ? "opacity-100" : "opacity-0") : "opacity-100")}
      style={{
...
export function ShowcaseCTA({ href, label, tone = "accent" }: { href: string; label: string; tone?: "accent" | "teal" | "ochre" }) {
  const toneClass = {
...
export function SceneGrid() {
  return (
    <div aria-hidden="true" className="pointer-events-none absolute inset-0 opacity-[0.1]" style={{ backgroundImage: "linear-gradient(to right, color-mix(in srgb, var(--dark-muted-foreground) 18%, transparent) 1px, transparent 1px), linear-gradient(to bottom, color-mix(in srgb, var(--dark-muted-foreground) 18%, transparent) 1px, transparent 1px)", backgroundSize: "52px 52px" }} />
  );
}
...
export function SceneFrameShell({ children }: { children: ReactNode }) {
  return (
    <div className="relative h-full min-h-0 w-full bg-[var(--dark-background)] text-[var(--dark-foreground)]">
      <SceneGrid />
      <div aria-hidden="true" className="pointer-events-none absolute inset-4 border border-[var(--dark-border)]/70 sm:inset-6 lg:inset-8" />
      <div className="relative h-full min-h-0 w-full p-6 sm:p-8 lg:p-12">{children}</div>
    </div>
  );
}
