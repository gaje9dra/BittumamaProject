    ...
    <SceneFrameShell>
      <div className="flex h-full flex-col gap-4 sm:gap-5">
        ...
        <div className="relative min-h-0 flex-1 overflow-hidden rounded-[var(--radius-lg)] border border-[var(--dark-border)] bg-[var(--dark-surface)]/45 p-3 sm:p-5">
          ...
          <div className="absolute left-0 top-0 w-[43%] sm:w-[29%]"><TopicButton topic={topics[0]} active={activeTopic === "thesis"} onClick={() => setActiveTopic("thesis")} /></div>
          ...
          <div className="absolute left-1/2 top-1/2 w-[54%] -translate-x-1/2 -translate-y-1/2 sm:w-[39%] lg:w-[34%]">
...
function TopicButton({ topic, active, onClick }: { topic: (typeof topics)[number]; active: boolean; onClick: () => void }) {
  return <button type="button" aria-pressed={active} onClick={onClick} className={cn("w-full rounded-[var(--radius-md)] border p-2 text-left transition-[border-color,background-color,transform] duration-200 sm:p-3", active ? "border-[var(--dark-border)] bg-[var(--dark-surface)] -translate-y-0.5" : "border-[var(--dark-border)]/70 bg-[var(--dark-background)]/70 hover:-translate-y-0.5 hover:border-[var(--dark-border)]")}>
