...
export function HomeAudience({ services }: { services: Service[] }) {
  ...
  return (
    <section id="home-audience" aria-labelledby="home-audience-title" className="scroll-anchor bg-surface-muted">
        ...
        <ScrollTransition distance={56}>
          <div className="grid gap-10 lg:grid-cols-12 lg:gap-x-8 xl:gap-x-12">
            <div className="lg:col-span-4">
              ...
              <Heading id="home-audience-title" level={2} className="mt-4 max-w-[18ch]">{audience.title}</Heading>
              <p className="type-body-sm mt-5 max-w-[38ch] text-muted-foreground">{audience.description}</p>
            </div>
                ...
                {audience.items.map((item) => (
                  <li key={item.index} className="grid gap-5 border-b border-border py-7 sm:grid-cols-[3rem_minmax(0,1fr)_minmax(15rem,.65fr)] sm:gap-6 sm:py-8">
                    ...
                    <div>
                      ...
                      <p className="type-body-sm mt-2 max-w-[42ch] text-muted-foreground">{item.description}</p>
                    </div>
                    <ul className="grid gap-1.5 sm:border-l sm:border-border sm:pl-6">
                      {item.serviceIds.map((serviceId) => {
                        const service = services.find(
                          (candidate) => candidate.id === serviceId || candidate.slug === serviceId,
                        );
