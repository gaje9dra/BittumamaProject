...
const modules = [
  { id: "question", label: "Research Question", short: "QUESTION" },
  { id: "literature", label: "Literature", short: "LITERATURE" },
...
export function ResearchWorkflowScene({ active }: { active: boolean }) {
  const [activeModule, setActiveModule] = useState<ModuleId>("methodology");

  useEffect(() => {
    if (active) setActiveModule("methodology");
  }, [active]);
    ...
    <SceneFrameShell>
      <div className="flex h-full flex-col gap-4 sm:gap-5">
        <header className="flex items-center justify-between gap-3">
          ...
          <span className="hidden text-[10px] uppercase tracking-[0.16em] text-[var(--dark-muted-foreground)] sm:block">Draft 04 · methodology</span>
        </header>
            ...
            <div className="rounded-[var(--radius-lg)] border border-[var(--dark-border)] bg-[var(--dark-surface)]/95 p-3 shadow-[var(--shadow-lg)] backdrop-blur-sm sm:p-4">
              ...
              <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-[1fr_0.7fr] sm:gap-3">
                <div className="rounded border border-[var(--dark-border)] bg-[var(--dark-background)] p-2.5">
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-1.5"><span className="h-9 rounded border border-[var(--dark-border)] bg-[var(--dark-surface)]" /><span className="h-9 rounded border border-[var(--dark-border)] bg-[var(--dark-surface)]" /><span className="h-9 rounded border border-[var(--dark-border)] bg-[var(--dark-surface)]" /></div>
                </div>
                <div className="rounded border border-[var(--dark-border)] bg-[var(--dark-background)] p-2.5">
                  <p className="text-[8px] uppercase tracking-[0.14em] text-[var(--dark-muted-foreground)]">Evidence map</p>
                  <svg aria-hidden="true" viewBox="0 0 180 100" className="mt-2 h-20 w-full"><path d="M10 80 C45 50 55 75 82 42 S120 64 170 18" fill="none" stroke="var(--data-2)" strokeWidth="3" /><circle cx="10" cy="80" r="4" fill="var(--data-2)" /><circle cx="82" cy="42" r="4" fill="var(--data-3)" /><circle cx="170" cy="18" r="4" fill="var(--data-5)" /><path d="M10 90H170M10 90V10" stroke="var(--dark-border)" strokeWidth="1" /></svg>
        ...
        <footer className="flex items-center justify-between gap-4 border-t border-[var(--dark-border)] pt-3">
          <div className="flex min-w-0 items-center gap-2 text-[9px] uppercase tracking-[0.12em] text-[var(--dark-muted-foreground)] sm:text-[10px]"><span className="h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--data-5)]" /><span className="truncate">Workflow state · {modules.find((item) => item.id === activeModule)?.short}</span></div>
