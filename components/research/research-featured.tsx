        ...
        <div className="grid gap-8 lg:grid-cols-12 lg:gap-x-8 xl:gap-x-12">
          ...
          <article className="lg:col-span-8 lg:col-start-4">
            <div className="grid gap-6 border-t border-border pt-6 sm:grid-cols-[minmax(0,1fr)_auto] sm:gap-10">
              <div>
                <Heading id="featured-research-title" level={2} className="max-w-[24ch]">{entry.title}</Heading>
                <p className="type-body-sm mt-4 max-w-[58ch] text-muted-foreground">{entry.summary ?? entry.shortDescription}</p>
                <dl className="mt-6 flex flex-wrap gap-x-6 gap-y-2">
                  <div><dt className="type-caption text-muted-foreground">Type</dt><dd className="type-body-sm mt-1">{entry.type ?? "Research"}</dd></div>
                  <div><dt className="type-caption text-muted-foreground">Category</dt><dd className="type-body-sm mt-1">{entry.category}</dd></div>
                  {entry.date && <div><dt className="type-caption text-muted-foreground">Date</dt><dd className="type-body-sm mt-1">{entry.date}</dd></div>}
                </dl>
              </div>
              <div className="sm:self-end">
                <Link href={getResearchHref(entry)} className="group inline-flex min-h-11 items-center gap-2 type-button text-primary underline decoration-primary/30 underline-offset-4 hover:decoration-primary focus-visible:outline-2 focus-visible:outline-offset-3">
                  View research
