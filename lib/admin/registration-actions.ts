  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    try {
      await prisma.client.$transaction(async (tx) => {
        ...
        await recordAudit(tx, { action: nextStatus === EventRegistrationRecordStatus.CANCELLED ? AuditAction.REGISTRATION_CANCELLED : AuditAction.REGISTRATION_STATUS_CHANGED, category: AuditCategory.REGISTRATION, result: AuditResult.SUCCESS, summary: nextStatus === EventRegistrationRecordStatus.CANCELLED ? "Registration cancelled by an administrator." : "Registration status changed.", entityType: "EventRegistration", entityId: registration.id, metadata: { registrationId: registration.id, eventId, previousStatus: registration.status, newStatus: nextStatus }, actor: { userId: null, type: "SYSTEM" }, metadata: { registrationId: registration.id, eventId, previousStatus: registration.status, newStatus: nextStatus, adminAccountId: actor.id } });
